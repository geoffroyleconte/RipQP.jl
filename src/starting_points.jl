function starting_points!(pt0 :: Point{T}, fd :: Abstract_QM_FloatData{T}, id:: QM_IntData, itd :: IterData{T}) where {T<:Real}
    ...
    dual_val = @avx itd.Qx .- itd.ATy .+ fd.c
    pt0.s_l = dual_val[id.ilow]
    pt0.s_u = -dual_val[id.iupp]

    # check distance to bounds δ for x, s_l and s_u
    @avx itd.x_m_lvar .= @views pt0.x[id.ilow] .- fd.lvar[id.ilow]
    @avx itd.uvar_m_x .= @views fd.uvar[id.iupp] .- pt0.x[id.iupp]
    if id.nlow == 0
    ...
    @avx itd.x_m_lvar .+= δx_l1
    @avx itd.uvar_m_x .+= δx_u1
    s0_l1 = @avx pt0.s_l .+ δs_l1
    s0_u1 = @avx pt0.s_u .+ δs_u1
    ...
    δx = max(δx_l2, δx_u2)
    δs = max(δs_l2, δs_u2)
    @avx pt0.x[id.ilow] .+= δx
    @avx pt0.x[id.iupp] .-= δx
    @avx pt0.s_l .= pt0.s_l .+ δs
    @avx pt0.s_u .= pt0.s_u .+ δs
